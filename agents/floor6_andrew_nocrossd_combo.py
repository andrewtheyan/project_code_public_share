...
class Agent(object):
    def __init__(self, agent_number, params={}):
        ...
        self.discount_q_0, self.discount_q_1 = 0.95, 0.95
        
        self.use_q_table = False
        ...
        self.q_table_file = 'machine_learning_model/q_table_combo1'
    ...
    def get_last_state_action(self):
        ...
        if self.did_customer_buy_from_me:
            self.last_reward = 5
            #self.alpha = self.alpha * 1.1#001
        else:
            self.last_reward = -10
            #self.alpha = self.alpha * 0.9#99
    ...
    def use_action(self, action):#, item_0_loss, item_1_loss
        ...
        #i0_diff = np.sum(item_0_loss) / len(item_0_loss)
        #i1_diff = np.sum(item_1_loss) / len(item_1_loss)
        ...
        if self.opponent_last_prices[0] < self.my_last_prices[0]:
            ...
            if down_p1 <= 0:
                down_p1 = self.opponent_last_prices[0] / self.my_last_prices[0] * 0.85
        ...
        if self.opponent_last_prices[1] < self.my_last_prices[1]:
            ...
            if down_p2 <= 0:
                down_p2 = self.opponent_last_prices[1] / self.my_last_prices[1] * 0.85
        ...
        if action == 0:
            return self.i0_ratio, self.i1_ratio
        if action == 1:
            return self.i0_ratio, up_p2
        if action == 2:
            return up_p1, self.i1_ratio
        if action == 3:
            return up_p1, up_p2
        if action == 4:
            return self.i0_ratio, down_p2
        if action == 5:
            return down_p1, self.i1_ratio
        if action == 6:
    ...
    def _process_last_sale(self, last_sale, profit_each_team):
        ...
        data = [self.which_item_customer_bought,x,y,self.my_last_prices,self.opponent_last_prices,my_current_profit, opponent_current_profit]
    ...
    def analysis(self,old_list, new_list):
        if old_list is None:
            ...
            # if Im performing really raelly well, try incraesing my price
                ...
                #self.discount_q_0, self.discount_q_1 = self.use_action(action, item_0_loss, item_1_loss)
    ...
    def action(self, obs):
        ...
        #if self.use_q_table:
        action = self.choose_action()
        self.discount_q_0, self.discount_q_1 = self.use_action(action)
        y = y*self.discount_q_0 #+ self.curr_alpha0
        ...
        #if self.use_q_table:
        x = x*self.discount_q_1 #+ self.curr_alpha1
        #else:
