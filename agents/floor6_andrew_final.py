    ...
    def analysis(self,old_list, new_list):
        if old_list is None:
            ...
            if n_oppo_0 - n_me_0 >= 1:
                if len(temp0) != 0:
                    avg0 = max(sum(item_0_loss)/len(item_0_loss), 0.07)
            ...
            if n_oppo_1 - n_me_1 >= 1:
                if len(temp1) != 0:
                    avg1 = max(sum(item_1_loss)/len(item_1_loss),0.07)
            ...
            # if my_last_block_profit < 1:
            #     self.curr_alpha1 += -2000
            #     self.curr_alpha0 += -2000
